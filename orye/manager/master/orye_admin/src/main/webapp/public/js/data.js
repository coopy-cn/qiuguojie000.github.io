...
//}
/*折线图switch切换start*/
$(".tabChange .count").on("click", function() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(!!data && data.resultCode == 200) {
				...
				var s = (successNum/totalNum).toFixed(4)*100;
				if(totalNum == 0){
					$(".index-4 .index-1-data").text("0%");
				}else{
					$(".index-4 .index-1-data").text(Math.floor(s * 100) / 100+"%");
				}
				$(".index-5 .index-1-data").text(data.payNet);
				$(".index-6 .index-1-data").text(data.rateAmount);
				$(".index-7 .index-1-data").text(data.rateProfit);
			} else {
				if(!!data.message) {
...
function charts() {
	...
	option = {
		...
		yAxis:  [{
			type: 'value',name:"金额",position:'left',min:0
		},{type: 'value',name:"笔数",position:'right',splitLine:{show: false},min:0}
		,{show:false,type: 'value',name:"成功率%",position:'left',offset:50,splitLine:{show: false},min:0}],
		series: [{
			name: '成功交易金额（元）',
			type: 'line',
			itemStyle: {
	...
	function linedata() {
		...
		ldata.payWayId = cdata.payWayId;
		ldata.rateId = cdata.rateId;
...
//							}
							...
							ydata3.push(data[i].successRate);
						}						
						option.series[0].data = ydata1;
						option.series[1].data = ydata2;
						option.series[2].data = ydata3;
						//console.log(ydata1);
...
function dateParam(){		
	if(cdata.dateType == 1){			
	    ...
	    cdata.endTime = y+"-"+m+"-"+d + " 23:59:59";
	    cdata.startTime = y+"-"+m+"-"+d + " 00:00:00";
		...
		cdata.endTime = y2+"-"+m2+"-"+d2+ " 23:59:59";

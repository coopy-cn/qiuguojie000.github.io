 ...
 * <P>@Description: 扩展Apache Commons BeanUtils, 提供一些反射方面缺失的封装.</P>
 * <P>Company:</P>
 * @author pengzhihao
 * @date 2015-11-28下午4:13:29
 */
public class BeanUtil extends BeanUtils {
    ...
    public static void copyPropertiesIgnoreNull(Object dest, Object orig) throws IllegalAccessException, InvocationTargetException {
    	copyPropertiesIgnoreNull(dest, orig, null);
    }
     ...
     * @param dest	目标
     * @param orig	来源
     * @param keepFileds	一定要复制字段
     * @throws IllegalAccessException
     * @throws InvocationTargetException
     */
    public static void copyPropertiesIgnoreNull(Object dest, Object orig,String[] keepFileds) throws IllegalAccessException, InvocationTargetException {
        List<String> keepList = null;
    	if(keepFileds!=null){
    		keepList = Arrays.asList(keepFileds);
        }
    	registerConverter();
        for(Field field : orig.getClass().getDeclaredFields()){
        	field.setAccessible(true);
        	if (field.get(orig)!=null||(keepList!=null&&keepList.indexOf(field.getName())>-1)){
        		BeanUtilsBean.getInstance().copyProperty(dest,field.getName(),field.get(orig));
			}
        }
    }
    ...
    public static <T> T getDeepClone(T obj) throws IOException, ClassNotFoundException {
        // 将对象写到流里

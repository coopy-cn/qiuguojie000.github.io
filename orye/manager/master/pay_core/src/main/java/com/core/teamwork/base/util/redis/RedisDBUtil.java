package com.core.teamwork.base.util.redis;
...
public class RedisDBUtil {
	 ...
	 * @date 2016年1月11日
	 */
	public static final String DEFAULT_REDIS_DAO_NAME = ReadPro.getValue(
			"redis_sentinel_masterName", "myPro");
	 ...
	 * @date 2016年1月11日
	 */
	public static final String DERALUT_REDIS_DAO_SENTINEL = ReadPro
			.getValue("redis_sentinel_array");
	 ...
	 * @date 2016年1月11日
	 */
	public static final String DEFAULT_REDIS_DAO_HOSTNAME = ReadPro
			.getValue("redis_host");
	 ...
	 * @date 2016年1月11日
	 */
	public static final Integer DEFAULT_REDIS_DAO_HOST_PORT = ReadPro.getValue(
			"redis_port", 6379);
	 ...
	 * @date 2016年1月11日
	 */
	public static final Integer DEFALUT_REDIS_DAO_DATABASE = ReadPro.getValue(
	 ...
	 * @date 2016年1月11日
	 */
	public static final String DEFAULT_REDIS_DAO_PWD = ReadPro
	 ...
	 * @Exception
	 */
	public static RedisDBUtil addNewRedisDaoBySentinel(String redisDaoName,
			String serverName, String sentinelArrayStr, Integer database,
			String pwd) {
		RedisDBUtil newRedisDao = new RedisDBUtil(redisDaoName, serverName,
				sentinelArrayStr, database, pwd);
	 ...
	 * @Exception
	 */
	public static RedisDBUtil addNewRedisDaoByOneHost(String redisDaoName,
			String redisHost, Integer database, String pwd, Integer port) {
		RedisDBUtil newRedisDao = new RedisDBUtil(redisDaoName, redisHost,
				database, pwd, port);
		redisDaoMap.put(newRedisDao.getRedisDaoName(), newRedisDao);
		return newRedisDao;
	}
	...
	private static Map<String, RedisDBUtil> initRedisMap() {
		...
		if (StringUtils.isNotBlank(DERALUT_REDIS_DAO_SENTINEL)) {
			redisDao = new RedisDBUtil(DEFAULT_REDIS_DAO_NAME,
					DEFAULT_REDIS_DAO_NAME, DERALUT_REDIS_DAO_SENTINEL,
					DEFALUT_REDIS_DAO_DATABASE, DEFAULT_REDIS_DAO_PWD);// 初始化默认的Redis哨兵连接
		} else if (StringUtils.isNotBlank(DEFAULT_REDIS_DAO_HOSTNAME)) {
			redisDao = new RedisDBUtil(DEFAULT_REDIS_DAO_NAME,
					DEFAULT_REDIS_DAO_HOSTNAME, DEFALUT_REDIS_DAO_DATABASE,
					DEFAULT_REDIS_DAO_PWD, DEFAULT_REDIS_DAO_HOST_PORT);
		}
	 ...
	 * 初始化一个使用哨兵连接的Redis
	 */
	private RedisDBUtil(String redisDaoName, String serverName,
			String sentinelArrayStr, Integer database, String pwd) {
		setRedisDaoName(redisDaoName);
		setJedisPool(getPoolBySentinel(serverName, sentinelArrayStr, database,
				pwd));
	}
	 ...
	 * @date 2016年1月11日
	 */
	private RedisDBUtil(String redisDaoName, String redisHost,
			Integer database, String pwd, Integer port) {
		setRedisDaoName(redisDaoName);
	...
	private Pool<Jedis> getPoolBySentinel(String serverName,
			String sentinelArrayStr, Integer database, String pwd) {
		Pool<Jedis> pool = null;
		if (pool == null) {
			JedisPoolConfig config = getConfig();
			Set<String> sentinels = new HashSet<String>();
			List<JSONObject> sentinelList = JSONArray.parseArray(
					sentinelArrayStr, JSONObject.class);
			for (JSONObject sentinel : sentinelList) {
				sentinels.add(new HostAndPort(sentinel.getString("address"),
						sentinel.getIntValue("port")).toString());
			}
			pool = new JedisSentinelPool(serverName, sentinels, config, 100000,
					pwd, database);
		}
		return pool;
	}

	private Pool<Jedis> getPoolByOneHost(String redisHost, Integer database,
			String pwd, Integer port) {
		JedisPoolConfig config = getConfig();
		Pool<Jedis> pool = new JedisPool(config, redisHost, port, 100000, pwd,
				database);
		return pool;
	}
				...
				try {
					...
					if (errorTimes == 3) {// 3次全部出错，表示服务器出现问题
						...
						if (ALIVE == false) {
							...
							// 修改休眠时间为30秒，尽快恢复服务
							sleepTime = 30000;
							logger.error("redis[" + redisDaoName
									+ "] 服务器恢复正常！ ！ ！");
						}
	 ...
	 * @param valueMap
	 */
	public List<String> hmsetList(Set<String> keyList,
	 ...
	 * @param valueMap
	 */
	public List<Long> addToSortedSet(Set<String> keyList,
	...
	public List<Long> addToSortedSet(String key,
			List<Map<String, Double>> valueMap) {
		Jedis conn = getConnection();
		if (conn == null) {
			return null;
		}
	...
	public Set<String> getZrangeByScore(String sortKey, String min, String max,
		...
		try {
			Set<String> result = conn.zrangeByScore(sortKey, min, max, offset,
					count);
			returnConnection(conn);
			return result;
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public List<Map<String, String>> hmGetByList(List<String> keys,
			String[] fields) {
		Jedis conn = getConnection();
		if (conn == null) {
			return null;
	 ...
	 * @return
	 */
	public List<Map<String, String>> hmsGetByList(List<String> keys,
	...
	public Set<Tuple> zrevrangeByScoreWithScores(String sortKey, int start,
			int size) {
		Jedis conn = getConnection();
		if (conn == null) {
			return null;
		...
		try {
			Set<Tuple> result = conn.zrevrangeByScoreWithScores(sortKey, start,
					start + size - 1);
			returnConnection(conn);
			return result;
		} catch (Exception e) {
	...
	public Set<Tuple> zrevrangeByScoreWithScores(String sortKey, String max,
			String min) {
		Jedis conn = getConnection();
		if (conn == null) {
			return null;
		...
		try {
			Set<Tuple> result = conn.zrevrangeByScoreWithScores(sortKey, max,
					min);
			returnConnection(conn);
			return result;
		} catch (Exception e) {
	...
	public List<Set<String>> zrevrangeByKeys(List<String> keys, long start,
			long end) {
		Jedis conn = getConnection();
		if (conn == null) {
			return null;
	...
	public List<String> zrange(String key, Long start, Long end) {
		...
		try {
			List<String> result = new ArrayList<String>(conn.zrange(key, start,
					end));
			returnConnection(conn);
			return result;
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public List<List<String>> lrangeByKeyList(List<String> keys, Long start,
			Long end) {
		Jedis conn = getConnection();
		if (conn == null) {
			return null;
	...
	public <T> List<Long> zaddByList(String key, Map<T, Double> scoreAndMember) {
		...
		try {
			...
			for (T member : scoreAndMember.keySet()) {
				list.add(p.zadd(key, scoreAndMember.get(member),
						member.toString()));
			}
			p.sync();
			List<Long> resultList = new ArrayList<Long>();
	 ...
	 * @return
	 */
	public List<Long> zCountByKeysAndScore(List<String> keys,
			Double scoreStart, Double scoreEnd) {
		Jedis conn = getConnection();
		if (conn == null) {
			return null;
		}
	...
	public Set<String> zByScoreWithScores(String sortKey, Double max, Double min) {
		...
		try {
			Set<String> result = conn
					.zrangeByScore(sortKey, max + "", min + "");
			returnConnection(conn);
			return result;
		} catch (Exception e) {
			returnBorkenConnection(conn);

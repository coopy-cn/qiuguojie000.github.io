...
import com.core.teamwork.base.j2cache.ehcache.EhCacheProvider;
...
//			Properties props = J2Cache.getConfig();
			...
			CacheManager.l1_provider = getProviderInstance(ReadPro.getValue("cache.L1.provider_class"));
			CacheManager.l1_provider.start();
			log.info("Using L1 CacheProvider : " + l1_provider.getClass().getName());
			
			CacheManager.l2_provider = getProviderInstance(ReadPro.getValue("cache.L2.provider_class"));
...
//	}
	 ...
	 * @param value Cache value
	 */
	public final static void setExpire(int level, String name, Object key, Object value,Integer expire){
		//System.out.println("SET => " + name+":"+key+"="+value);
		if(name!=null && key != null && value!=null) {
            Cache cache =_GetCache(level, name, true);
            if (cache != null)
                cache.putExpire(key,value,expire);
        }
	}

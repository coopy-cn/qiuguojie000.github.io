 ...
 */
package com.core.teamwork.base.j2cache.ehcache;
...
public class EhCacheProvider implements CacheProvider {
	...
	public void start() throws CacheException {
		...
		// 如果指定了名称,那么尝试获取已有实例
		String ehcacheName = ReadPro.getValue(KEY_EHCACHE_NAME);
		if (ehcacheName != null && ehcacheName.trim().length() > 0)
			manager = CacheManager.getCacheManager(ehcacheName);
		if (manager == null) {
			// 指定了配置文件路径? 加载之
			String ehcacheconfigXml = ReadPro.getValue(KEY_EHCACHE_CONFIG_XML);
			if (null != ehcacheconfigXml) {
				manager = new CacheManager(ehcacheconfigXml);
			} else {
				// 加载默认实例
